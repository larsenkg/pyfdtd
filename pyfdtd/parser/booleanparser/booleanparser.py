...
class BooleanParser:
    ...
    def parse_operators(self, expr, kargs):
        ...
        def callback_not(x, kargs):
            ...
            return self.unary_operator(
                x, "not", numpy.logical_not, callback_not_parsable, kargs
            )
        ...
        def callback_and(x, kargs):
            ...
            return self.binary_operator(
                x, None, "and", numpy.logical_and, callback_not, kargs
            )
        ...
        def callback_or(x, kargs):
            # parse or
            return self.binary_operator(
                x, None, "or", numpy.logical_or, callback_and, kargs
            )

        # try parse xor
        result = self.binary_operator(
            expr, None, "xor", numpy.logical_xor, callback_or, kargs
        )
    ...
    def brackets(self, expr, excludes=[], callback=None, kargs={}):
        ...
        if openingBracket != 0:
            if expr[openingBracket - 1] != " " and expr[openingBracket - 1] != "(":
                # not valid
                return self.brackets(
                    expr, excludes + [(openingBracket, closingBracket)], callback, kargs
                )

        # add new variable to kargs
        varname = "v" + "{}".format(random.randint(1, 1000000))
        kargs[varname] = callback(expr[openingBracket + 1 : closingBracket], kargs)
    ...
    def binary_operator(self, x1, x2, keyword, function, callback, kargs):
        ...
        if isinstance(x1, str):
            ...
            if len(expressions) > 1:
                x1 = self.binary_operator(
                    (expressions[0]).strip(),
                    keyword.join(expressions[1:]).strip(),
                    keyword,
                    function,
                    callback,
                    kargs,
...
if __name__ == "__main__":
    ...
    X, Y = numpy.meshgrid(numpy.arange(0.0, 1.0, 0.1), numpy.arange(0.0, 1.0, 0.1))
